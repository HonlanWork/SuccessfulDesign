...
class ContestController extends CommonController {
	...
	public function promotion_pay(){
		...
		switch ($channel) {
		    case 'alipay_wap':
		        $extra = array(
		            'success_url' => C('SITE_PREFIX') . U('Pay/promotion_pay_success', array('submission_id'=>$submission_id,'promotion_code'=>$orderNo)),
		            'cancel_url' => C('SITE_PREFIX') . U('Pay/promotion_pay_cancel', array('submission_id'=>$submission_id,'promotion_code'=>$orderNo))
		        );
		        break;
		    case 'bfb_wap':
		        $extra = array(
